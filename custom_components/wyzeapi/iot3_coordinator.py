...
class WyzeLockBoltV2Coordinator(DataUpdateCoordinator):
    ...
    async def lock_unlock(self, command: str):
        """Execute lock or unlock command and raise on failure so callers can react."""
        ...
        try:
            ...
            if result.get("code") != "1":
                ...
                _LOGGER.error("Lock %s command failed for %s: %s", command, self._lock.nickname, msg)
                raise HomeAssistantError(f"IoT3 command failed: {msg}")
        except HomeAssistantError:
            raise
        except Exception as exc:
            _LOGGER.exception("Failed to %s lock %s", command, self._lock.nickname)
            raise HomeAssistantError(f"Failed to {command} lock: {exc}") from exc
        finally:
